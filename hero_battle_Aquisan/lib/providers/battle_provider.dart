...
class BattleProvider extends ChangeNotifier {
  ...
  Future<void> executeTurn() async {
    if (isBattleOver || currentPlayerHero == null || currentAiHero == null) return;

    // Speed check: Player goes first if their speed is greater or equal
    if (currentPlayerHero!.speed >= currentAiHero!.speed) {
  ...
  void _resolveStrike({required bool attackerIsPlayer}) {
    ...
    final damage = BattleEngine.calculateDamage(attacker.attack, defender.defense);

    if (attackerIsPlayer) {
      aiHp -= damage;
      battleLog.insert(0, '${attacker.name} hits ${defender.name} for $damage damage!');
      if (aiHp <= 0) {
