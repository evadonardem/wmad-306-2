...
class HeroModel {
	...
	factory HeroModel.fromJson(Map<String, dynamic> json) {
		var imageUrl = (json['image'] as Map?)?['url'] as String? ?? '';
		// Clean up and validate the image URL
		if (imageUrl.isNotEmpty && !imageUrl.startsWith('http')) {
			imageUrl = '';
		}
		// If image URL is empty, generate a fast avatar placeholder using ui-avatars
		if (imageUrl.isEmpty) {
			final name = json['name'] as String? ?? 'Hero';
			imageUrl = 'https://ui-avatars.com/api/?name=${Uri.encodeComponent(name)}&size=400&background=random&bold=true&rounded=true';
		}
		return HeroModel(
			id: json['id'].toString(),
