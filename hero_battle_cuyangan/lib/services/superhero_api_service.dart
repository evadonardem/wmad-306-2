...
class SuperheroApiService {
  ...
  Future<List<HeroModel>> fetchAllHeroes() async {
    ...
    try {
      ...
      // Add debug print to see if data is arriving
      print('Heroes loaded: ${heroes.length}');
  ...
  Future<List<HeroModel>> searchHeroes(String name) async {
    try {
      ...
      print('Search results for "$name": ${searchResults.length}');
