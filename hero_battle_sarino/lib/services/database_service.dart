...
class DatabaseService {
  ...
  Database? _db;
  static final List<BattleRecord> _fallbackHistory = [];
...
played_at TEXT NOT NULL
  ...
  Future<void> saveBattleRecord(BattleRecord record) async {
    try {
      ...
      // Database is unavailable on some platforms (web), so store in memory.
      _fallbackHistory.insert(0, record);
    }
  }
  ...
  Future<List<BattleRecord>> loadHistory() async {
    try {
      ...
      // Return fallback history if the database cannot be opened.
      return List.unmodifiable(_fallbackHistory);
    }
  }
}
